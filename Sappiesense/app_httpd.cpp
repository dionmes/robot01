...
#define SD_CONFIG SdSpiConfig(14, DEDICATED_SPI, SPI_CLOCK)
#else  // HAS_SDIO_CLASS
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    uint8_t *out_buf;
    bool s;
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFOw
    bool detected = false;
#endif
    int face_id = 0;
...
////////////////////////////////////////////////////////////
/// Custom hanlder functions ///////////////////////////////
////////////////////////////////////////////////////////////

static esp_err_t micstream_handler(httpd_req_t *req)
{   
    char *buf = NULL;
    char value[2];
    ...
    // Start mic streaming
    if (httpd_query_key_value(buf, "on", value, sizeof(value)) == ESP_OK) {    
      int on = atoi(value);

      if (on == 1 && !micIsStreaming) {
        ...
        String response = "Mic streaming to IP: " + ip + ", PORT: " + outPort;
...
static esp_err_t micgain_handler(httpd_req_t *req)
{   
    char *buf = NULL;
    char value[8];
    ...
    // Start mic streaming
    if (httpd_query_key_value(buf, "gain", value, sizeof(value)) == ESP_OK) {    
      int gain = atoi(value);

      if (gain >= 0 and gain <= 50 ) {
        gain_factor = gain;
        const char resp[] = "Gain set";
        httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
        return httpd_resp_send(req,resp , HTTPD_RESP_USE_STRLEN);
...
void startCameraServer()
{
  ...
  /////// Custom Sappie start ///////////////////////////////////
  ///////////////////////////////////////////////////////////////
  // Load plain text config file called "sappie.cfg" from sdcard
  // Without any formatting or seperating other than new line.
  // first line : ssid, second line: password
  // third line : destination ip for mic recording.
  ///////////////////////////////////////////////////////////////

  Serial.println("Opening SD card\n");
  sd.begin(21);

  const size_t LINE_DIM = 40;
  char ssid[LINE_DIM];
  char password[LINE_DIM];
  char remoteMicIP[LINE_DIM];

  if (!file.open("sappie.cfg", O_READ))  {
    log_e("Cannot open sappie.cfg\n");
  }

  file.fgets(ssid, sizeof(ssid));
  ssid[strlen(ssid)-1] = 0;
  file.fgets(password, sizeof(password));
  password[strlen(password)-1] = 0;
  file.fgets(remoteMicIP, sizeof(remoteMicIP));
  remoteMicIP[strlen(remoteMicIP)-1] = 0;

  outIp.fromString(remoteMicIP);
...
///////////// for Sappie.     /////////////////
    ...
    httpd_uri_t micstream_url = {
        .uri = "/micstream",
        .method = HTTP_GET,
        .handler = micstream_handler,
        .user_ctx = NULL
    ...
    httpd_uri_t micgain_url = {
        .uri = "/micgain",
        .method = HTTP_GET,
        .handler = micgain_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
    ...
    httpd_uri_t go2sleep_url = {
        .uri = "/go2sleep",
        .method = HTTP_GET,
        .handler = go2sleep_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
...
//////////////////////////////////////////////////
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        // Custom handlers
        httpd_register_uri_handler(camera_httpd, &micstream_url);
        httpd_register_uri_handler(camera_httpd, &micgain_url);
        httpd_register_uri_handler(camera_httpd, &go2sleep_url);
